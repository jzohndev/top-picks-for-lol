...
public class FragmentPopular extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        View rootView = inflater.inflate(R.layout.fragmentpopular_layout, container, false);
        setRetainInstance(true);
        try {
            MainHandler x = new MainHandler();
            champArray = x.getTopPickChamps();
            champArrayPercent = x.getTopPickChampsPercent();
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    private View uiAssigner(View rView) {
        ...
        for (int i = 0; i < 10; i++) {
            ...
            currentPercent.setText(champArrayPercent[i]);
